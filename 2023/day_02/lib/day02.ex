defmodule Day02 do
  ...
  by a semicolon-separated list of subsets of cubes that were revealed from the
  ...
  def play_part1(input) do
    input
    |> String.split("\n", trim: true)
    |> Enum.map(&Game.to_struct/1)
    |> Enum.filter(&valid_game?/1)
    |> Enum.map(& &1.id)
    |> Enum.sum()
  end

  defp valid_game?(%Game{sets: []}), do: true

  defp valid_game?(%Game{sets: [set | tail]} = game) do
    if valid_set?(set), do: valid_game?(%{game | sets: tail}), else: false
  end

  defp valid_set?(%{red: red, green: green, blue: blue}) do
    red <= @bag.red && green <= @bag.green && blue <= @bag.blue
  end

  def play_part2(input) do
    input
    |> String.split("\n", trim: true)
    |> Enum.map(&Game.to_struct/1)
  ...
  defp calculate_power(%Game{sets: sets} = game) do
    %{red: required_red} = Enum.max_by(sets, & &1.red)
    %{green: required_green} = Enum.max_by(sets, & &1.green)
    %{blue: required_blue} = Enum.max_by(sets, & &1.blue)

    Map.put(game, :power, required_red * required_green * required_blue)
  end
end
